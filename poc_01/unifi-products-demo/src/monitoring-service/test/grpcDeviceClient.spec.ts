...
import { GrpcDeviceClient } from "../clients/GrpcDeviceClient";
import { discoverProtocol, clientFor } from "../clients/clientFactory";
...
function grpcDeviceScript(name: string, port: number, extra = ""): string {
  ...
  return `
    const { startGrpcDevice } = require("./02-grpc/common/start-grpc");
    startGrpcDevice({
