...
## Documentation phase (architecture diagrams, user manual)

- Generated six diagrams (system architecture, state machine, data
  model, two sequence diagrams, deployment topology) with AI assistance,
  using Graphviz for the structural diagrams and a small hand-written
  SVG generator for the two sequence diagrams (mermaid-cli was
  available but its headless-browser dependency couldn't be downloaded
  in this development environment, so it wasn't used).
- Every diagram was rendered to PNG and visually reviewed before being
  accepted, not just generated and assumed correct. Two were rejected
  and rebuilt after review: an automatic Graphviz layout for the state
  machine produced overlapping, unreadable labels; a manual `neato`
  positioning attempt collapsed all three nodes into one point. The
  version actually shipped is a third attempt, hand-coordinated.
- Two smaller rendering bugs were caught the same way — a literal
  `\u2014` printing instead of an em dash, and literal `\n` characters
  printing instead of line breaks — both found by looking at the
  rendered image, not by reading the generating code.
- `docs/architecture.md` and `docs/user-manual.md` were checked
  programmatically, not just proofread: every table-of-contents anchor
  was verified to match its heading's actual generated slug (an error
  made twice earlier in this same project, from assuming GitHub
  collapses an em-dash-adjacent double space into a double hyphen
  rather than a single one), and every embedded image path was
  confirmed to resolve to a real file.
