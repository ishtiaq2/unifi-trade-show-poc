...
| [`docs/requirements.md`](docs/requirements.md) | Stated vs. implied requirements, prioritised |
...
| **[`docs/architecture.md`](docs/architecture.md)** | The same architecture, as diagrams — system layers, state machine, data model, two sequence flows, deployment topology |
| **[`docs/user-manual.md`](docs/user-manual.md)** | How to run it, register devices, read status, tune for a bad venue network, troubleshoot |
| [`docs/non-goals.md`](docs/non-goals.md) | What was deliberately left out, and the reasoning |
| [`devices/README.md`](devices/README.md) | The 6 device simulators — setup, podman/podman-compose, troubleshooting |
| [`monitoring-service/README.md`](monitoring-service/README.md) | The service itself — architecture, testing, flaky tests found and fixed |
...
and all six simulated devices. Then register the devices and look at
them:
...
# 2. Devices (each in its own terminal)
...
integration against real running device simulators over real sockets,
and a life-cycle suite that boots the actual entrypoint as a subprocess
and shuts it down through the real SIGTERM handler. Nothing about the
...
devices/              6 simulators — router, switch, 2×camera, 2×door-access
                       REST + gRPC, own README/Dockerfile/docker-compose.yml
db/init.sql            schema
...
customer costs far more than briefly showing a recovered one as
suspect. Reasoning in [`docs/assumptions.md`](docs/assumptions.md) #4.

## Known gaps

- **The container build has not been executed in this development
  environment** — no registry access here. The compiled build it runs
  was verified directly (`npm run build && node dist/index.js`);
  confirm `docker compose up` / `podman-compose up` on your machine
  before relying on it at the venue.
- **gRPC is verified against simulators only** — no gRPC hardware
  exists in this scenario (`docs/assumptions.md` #2).
